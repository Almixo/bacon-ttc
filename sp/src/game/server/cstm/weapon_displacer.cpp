...
constexpr auto CHARGEUP		= SINGLE;
...
void CDisplacer::SecondaryAttack(void)
{
	...
	auto pEnt = gEntList.FindEntityByClassnameNearest( ENTNAME, pOwner->GetAbsOrigin(), RADIUS );
	if ( !pEnt )
	{
		WeaponSound(DENY);
		pOwner->SetNextAttack(gpGlobals->curtime + 1.5f);
	...
	fNextThink = gpGlobals->curtime + 1.5f;

	pFound = pEnt;

	iModeSwitch = SECONDARY;
...
void CDisplacer::PrimaryAttack(void)
{
	...
	fNextThink = gpGlobals->curtime + SequenceDuration();
