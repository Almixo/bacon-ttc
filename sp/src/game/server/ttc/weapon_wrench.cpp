...
void CWeaponWrench::ItemPostFrame()
{
	...
	if ((pPlayer->m_nButtons & IN_ATTACK2) && m_iWrenchStage == 0 && m_flStageUpdate < gpGlobals->curtime && m_flNextSecondaryAttack < gpGlobals->curtime)
	{
		m_iWrenchStage = 1;
	...
	if (m_iWrenchStage == 1 && m_flHoldUpdate < gpGlobals->curtime)
	{
	...
	if (!(pPlayer->m_nButtons & IN_ATTACK2) && m_iWrenchStage == 1 && m_flStageUpdate < gpGlobals->curtime)
	{
		m_iWrenchStage = 0;
		Swing(true);
...
void CWeaponWrench::Swing(int bIsSecondary)
{
	trace_t traceHit;
...
//------------------------------------------------------------------------------
void CWeaponWrench::Hit(trace_t &traceHit, Activity nHitActivity, bool bIsSecondary)
{
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
	if (!pPlayer)
		return;
